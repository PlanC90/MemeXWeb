...
const RoadmapItem: React.FC<RoadmapItemProps> = ({ phase, description, timeline, isCurrent, position, isCompleted }) => {
  const dotColorClass = isCompleted ? 'bg-green-500 border-green-600' : 'bg-black border-gray-600';
  const textColorClass = isCompleted ? 'text-green-400 line-through' : 'text-gray-400';
  const phaseColorClass = isCompleted ? 'text-green-500' : 'text-blue-500';

  return (
    <div className={`relative ${position === 'left' ? 'md:mr-12' : 'md:ml-12'}`}>
      <div className={`absolute left-1/2 transform -translate-x-1/2 -top-4 w-8 h-8 rounded-full border-4 ${dotColorClass}`}></div>
      <div className={`bg-gray-800/50 backdrop-blur-sm p-6 rounded-2xl border border-gray-700 hover:border-blue-500/50 transition-colors ${position === 'left' ? 'md:text-right' : 'md:text-left'}`}>
        <h3 className={`text-xl font-bold mb-2 ${phaseColorClass}`}>{phase}</h3>
...
const Roadmap = () => {
  const roadmapData = [
    { phase: "Phase 1: Launch & Airdrop", description: "Website launch, community building, and free airdrop distribution to early adopters.", timeline: "Completed", position: 'right', isCompleted: true },
    { phase: "Phase 2: 1 Million Holders", description: "Achieve a milestone of 1 million Memex token holders.", timeline: "Completed", position: 'left', isCompleted: true },
    { phase: "Phase 3: Web3 Wallet Integrations", description: "Integrate Memex with popular Web3 wallets for seamless access and utility.", timeline: "Completed", position: 'right', isCompleted: true },
